...
class SeeedB601RTFollower(Robot):
    ...
    def _render_rt_arm_yaml(self) -> str:
        ...
        lines = [
            ...
            f"rate: {fmt_float(self.config.rt_rate)}",
        ]
        ...
        for idx, motor_name in enumerate(self.motor_names):
            ...
            lines.extend(
                [
                    ...
                    f"      kd: {fmt_float(kd)}",
                    "    POS_VEL:",
                    f"      vel_kp: {fmt_float(vel_kp)}",
                    f"      vel_ki: {fmt_float(vel_ki)}",
                    f"      pos_kp: {fmt_float(pos_kp)}",
                    f"      pos_ki: {fmt_float(pos_ki)}",
                    f"      vlim: {fmt_float(vlim[idx])}",
                    "",
                ]
            )
        return "\n".join(lines)
