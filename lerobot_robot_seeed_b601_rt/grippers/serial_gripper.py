...
"""Pure-serial Xense gripper driver for Seeed B601 RT follower.
...
class SerialGripper:
    """Wrapper around XenseSerialGripper for use by the B601 RT follower.
    ...
    def __init__(self, config: SerialGripperConfig):
        ...
        label = config.sn or config.port.split("/")[-1] or "unconfigured"
        self._logger = logging.getLogger(f"{__name__}.SerialGripper-{label}")
    ...
    def connect(self) -> None:
        ...
        if not self._config.sn and not self._config.port:
        ...
        if self._init_open:
            try:
                ...
                self._logger.warning(f"Gripper init-open failed (non-fatal): {e}")
